...
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_1__libs_slick__ = __webpack_require__(337);
...
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_2__libs_masonry__ = __webpack_require__(338);
...
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_3__libs_jcf__ = __webpack_require__(339);
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_3__libs_jcf___default = __webpack_require__.n(__WEBPACK_IMPORTED_MODULE_3__libs_jcf__);
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_4__libs_jcf_select__ = __webpack_require__(340);
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_4__libs_jcf_select___default = __webpack_require__.n(__WEBPACK_IMPORTED_MODULE_4__libs_jcf_select__);
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_5__libs_binder__ = __webpack_require__(341);
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_6__modules_accordion__ = __webpack_require__(342);
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_7__modules_customYTPlayer__ = __webpack_require__(343);
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_8__modules_scrollAnimate__ = __webpack_require__(344);
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_9__modules_popups__ = __webpack_require__(345);
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_10__modules_backToTop__ = __webpack_require__(346);
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_11__modules_googleMap__ = __webpack_require__(347);
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_12__modules_vueMap__ = __webpack_require__(348);
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_13__modules_module__ = __webpack_require__(369);
...
Object(__WEBPACK_IMPORTED_MODULE_5__libs_binder__["a" /* default */])({
    bounds: {
        ...
        ".popups": __WEBPACK_IMPORTED_MODULE_9__modules_popups__["a" /* popups */],
        ".to-top": __WEBPACK_IMPORTED_MODULE_10__modules_backToTop__["a" /* backToTop */]
    },
    runTests: false
});
...
(function (window, factory) {
  ...
  if (true) {
    // AMD
    !(__WEBPACK_AMD_DEFINE_ARRAY__ = [__webpack_require__(23)], __WEBPACK_AMD_DEFINE_RESULT__ = (function (jQuery) {
...
function accordion() {
  ...
  var accordionItem = accordion.find(".accordion-item");
...
"use strict";
/* harmony export (immutable) */ __webpack_exports__["a"] = customYTPlayer;
function _toConsumableArray(arr) { if (Array.isArray(arr)) { for (var i = 0, arr2 = Array(arr.length); i < arr.length; i++) { arr2[i] = arr[i]; } return arr2; } else { return Array.from(arr); } }

function customYTPlayer() {
  var playBtn = document.querySelectorAll(".video .custom-play-btn");
  [].concat(_toConsumableArray(playBtn)).map(function (item) {
    item.addEventListener("click", function (event) {
      event.preventDefault();
      var poster = item.parentNode;
      var iframe = poster.parentNode.querySelector("iframe");
...
function scrollAnimate() {
  ...
  var addHeight = $(window).height() * .75;
  ...
  if (!this.isDev) {
    ...
    $(window).on("scroll", function () {
      ...
      for (var _i = 0; _i < elements.length; _i++) {
        var _elementPos = $(elements[_i]).offset().top;
        if (_elementPos < scrolled + addHeight) {
          $(elements[_i]).addClass("animation");
        }
      }
    });
  }
...
function popups() {
  var mainWrap = this.$body;
  var popupWrap = $(".popups");
...
"use strict";
/* WEBPACK VAR INJECTION */(function($) {/* harmony export (immutable) */ __webpack_exports__["a"] = backToTop;
function backToTop() {
...
function googleMap() {
    if ($("#map-home").length) {
        var createMap = function createMap() {
            var $map = $("#map-home");
            var image = void 0;
            var iconSrc = $map.attr("data-image");
            ...
            function addMarkers(coords, map, pin, title, content) {
                ...
                var info = new google.maps.InfoWindow({
                    position: coords,
                    map: map,
                    content: "<div class='map-note'>" + "<h5>" + title + "</h5>" + "<div class='content-note'>" + "<div>" + content + "</div>" + "</div>" + "</div>"
                });
...
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_0_vue_date__ = __webpack_require__(349);
    ...
    if ($("#app").length) {
        ...
        var getters = {
            ...
            filter_mapmenu_list: function filter_mapmenu_list(state) {
                return function (id) {
                    var id_new = Number(id);
                    if (id == 0) {
                        id_new = Number(id) + 1;
                    }
                    for (var idx in state.map_api) {
        ...
        var actions = {
            ...
            load_map_api: function load_map_api(_ref2, query) {
                var commit = _ref2.commit;

                var mapApi = mapApiUrl;
                __WEBPACK_IMPORTED_MODULE_1_axios___default.a.get(mapApi).then(function (data) {
                    commit("setMapList", { mapList: data.data });
                });
            }
        };
        var mutations = {
            setMapList: function setMapList(state, _ref3) {
                var mapList = _ref3.mapList;

                state.map_api = mapList;
                var mapApi = state.map_api;
                for (var item in mapApi) {
                    var itm = state.map_api[item];
                    state.map_api_points.push(itm);
                }
            },
            setEventList: function setEventList(state, _ref4) {
                var events = _ref4.events,
                    events_length = _ref4.events_length;
        ...
        Vue.component("mapHolder", {
            ...
            computed: {
                getNewLocations: function getNewLocations() {
            ...
            methods: {
                loadMapApi: function loadMapApi() {
                    this.$store.dispatch('load_map_api');
                },
                tabs: function tabs(index, sub_index) {
                            ...
                            function infoWindowOpen() {
                                ...
                                if (Object.keys(infoWindowContent).length !== 0) {
                                    ...
                                    if (typeof infoWindowContent.name === 'undefined') {
                                        infoWindowTemplate = "<div class='map-note-neighborhood'>" + "<div id='content-note'>" + "<div>" + infoWindowContent.address + "</div>" + "</div>" + "</div>";
                                    } else if (typeof infoWindowContent.address === 'undefined') {
                                        infoWindowTemplate = "<div class='map-note-neighborhood'>" + "<h6>" + infoWindowContent.name + "</h6>" + "</div>";
                                    } else {
                                        infoWindowTemplate = "<div class='map-note-neighborhood'>" + "<h6>" + infoWindowContent.name + "</h6>" + "<div id='content-note'>" + "<div>" + infoWindowContent.address + "</div>" + "</div>" + "</div>";
                                    }
...
"use strict";
/* WEBPACK VAR INJECTION */(function($) {/* harmony export (immutable) */ __webpack_exports__["a"] = popupGallery;
function popupGallery(finalCurrent) {
  var slider = $(".popup-gallery-slider");
